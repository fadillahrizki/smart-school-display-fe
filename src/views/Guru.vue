...
const onSubmit = () => {
  if (isEdit.value) {
    api.put(`/teachers/${teacher.value.id}`, teacher.value).then((response) => {
      open.value = false
      teacher.value = {
        name: ''
      }
    ...
    api.post('/teachers', teacher.value).then((response) => {
      open.value = false
      teacher.value = {
        name: ''
      }
        ...
        <div class="px-6 py-4 text-left modal-content">
          ...
          <div>
            ...
            <div>
              ...
              <input type="text" class="w-full p-2 border bg-white border-gray-200 rounded-md" v-model="teacher.name" />
            </div>

            <div class="mt-2">
              <label class="text-xs">Mata Pelajaran</label>
              <select name="mata_pelajaran" id="mata_pelajaran"
                class="w-full p-2 border bg-white border-gray-200 rounded-md" v-model="teacher.mata_pelajaran_id">
                <option value="" selected>Pilih Mata Pelajaran</option>
                <option v-for="mapel in mataPelajarans" :key="mapel.id" :value="mapel.id">
                  {{ mapel.name }}
                </option>
              </select>
            </div>
        ...
        <div class="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow sm:rounded-lg">
          <table class="min-w-full">
            ...
            <tbody class="bg-white">
              <tr v-for="(u, index) in teachers" :key="index">
